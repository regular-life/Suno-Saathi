...
class SunoSaarthiCore:
    ...
    def __init__(self):
        ...
        self.detector = WakeWordDetector(
            energy_threshold=settings.WAKE_WORD_ENERGY_THRESHOLD,
            pause_threshold=settings.WAKE_WORD_PAUSE_THRESHOLD,
        ...
        while True:
            try:
                if self.standby:
                    ...
                    recognized_text = command_result.get("command", "") if command_result.get("status") == "success" else ""
    ...
    async def process_command(self, command: str) -> Dict[str, Any]:
        ...
        if any(keyword in command.lower() for keyword in ["flyover", "shortcut", "traffic", "route", "turn", "destination"]):
            # Handle as a navigation query
            nav_result = self.navigation.process_navigation_query(
                query=command,
    ...
    async def set_navigation(self, origin: str, destination: str):
        ...
        try:
            ...
            directions = self.navigation.get_directions(
                origin=origin,
            ...
            return {
                "status": "error",
                "message": f"Error setting navigation: {str(e)}"
            }
